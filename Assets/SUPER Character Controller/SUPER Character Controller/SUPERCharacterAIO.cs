...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    void Update(){
        if(Vector3.Distance(p_Rigidbody.velocity, new Vector3(0f, 0f, 0f)) > 2.5) {
            _3rdPersonCharacterAnimator.SetBool("isRun",true); 
        }
        else {
            _3rdPersonCharacterAnimator.SetBool("isRun",false); 
        }
    ...
    void MovePlayer(Vector3 Direction, float Speed){
       // GroundInfo gI = GetGroundInfo();
        ...
        _2DVelocityMag = Mathf.Clamp((walkingSpeed/50) / _2DVelocity.magnitude, 0f,2f);
                _3rdPersonCharacterAnimator.SetTrigger("isRun"); 
    ...
    void Jump(float Force){
        if((currentGroundInfo.isInContactWithGround) && 
            ...
            (currentStance == Stances.Standing && !Jumped)){
                _3rdPersonCharacterAnimator.SetTrigger("jump");  
                Jumped = true;
