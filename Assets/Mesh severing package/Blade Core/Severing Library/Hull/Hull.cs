...
public class Hull
{
	//this values are for settin a default size in cut
	private static float smallestValidLength = 0.001f;
	private static float smallestValidRatio = 0.005f;
	...
	private void AssignPoints(Hull a, Hull b, Vector3 pointOnPlane, Vector3 planeNormal, out bool[] pointAbovePlane)
	{
		...
		foreach (Point point in points)
		{
			...
			if(Vector3.Dot(point.position - pointOnPlane, planeNormal) == 0.0f)
			{
				Debug.DrawRay(point.position,Vector3.up * 100,Color.red,3);
			}
		}
	}
	...
	private void SortCutEdges(IList<Edge> edgesA, IList<Edge> edgesB)
	{
		...
		//Debug.Log("Num of closed loops: " + clossedLoops);
	}
